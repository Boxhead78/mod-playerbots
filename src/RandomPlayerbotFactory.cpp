                                                                                                ...
                                                                                                uint8 gender)
{
    ...
    switch (race)
    {
        ...
        case RACE_WORGEN:     baseIndex = NameRaceAndGender::WorgenMale; break;
...
bool RandomPlayerbotFactory::IsValidRaceClassCombination(uint8 race, uint8 cls, uint32 expansion)
{
    ...
    // skip expansion races if not playing with expansion
    if (expansion < EXPANSION_CATACLYSM && (race == RACE_WORGEN || race == RACE_GOBLIN))
        return false;
...
void RandomPlayerbotFactory::CreateRandomBots()
{
    ...
    for (uint32 accountNumber = 0; accountNumber < totalAccountCount; ++accountNumber)
    {
        ...
        WorldSession* session = new WorldSession(accountId, "", 0x0, nullptr, SEC_PLAYER, EXPANSION_CATACLYSM,
