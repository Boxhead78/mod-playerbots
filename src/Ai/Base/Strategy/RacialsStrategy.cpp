...
void RacialsStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    ...
    if (botAI->HasSpell("rocket jump"))
    {
        triggers.push_back(new TriggerNode(
            "medium health", NextAction("rocket jump", ACTION_NORMAL + 5), nullptr));
    }

    if (botAI->HasSpell("feral lunge"))
    {
        triggers.push_back(new TriggerNode(
            "random", NextAction("feral lunge", ACTION_NORMAL + 5), nullptr));
    }

    if (botAI->HasSpell("darkflight"))
    {
        triggers.push_back(new TriggerNode(
            "often", NextAction("darkflight", ACTION_NORMAL + 5), nullptr));
    }

    if (botAI->HasSpell("rocket barrage"))
    {
        triggers.push_back(new TriggerNode(
            "enemy out of melee", NextAction("rocket barrage", ACTION_NORMAL + 5), nullptr));
    }
